...
bool s11n::list::serialize_list( NodeType & dest, const SerType & src )
{
	...
	if( ! TR::empty( dest ) )
	{
		throw ::s11n::s11n_exception( "serialize_list() requires that the target node be empty.", __FILE__, __LINE__ );
	}
...
#define ERRMSG S11N_TRACE(TRACE_WARNING) << "deserialize_list(node,list) srcnode="<<std::dec<<&src << ": "
	for( ; et != it; ++it )
	{
		...
		if( ! nch /* this never happens */ )
		{
			...
			throw ::s11n::s11n_exception( "Internal error: node children list contains a null pointer.", __FILE__, __LINE__ );
			return false;
		}
