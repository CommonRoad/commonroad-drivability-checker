...
PairType s11n::map::deserialize_streamable_pair( const NodeType & src  )
{
	...
	{
		throw ::s11n::s11n_exception( "deserialize_streamable_pair(): 'first' property not set!", __FILE__, __LINE__ );
	}
	if( ! TR::is_set( src, "second" ) )
	{
		throw ::s11n::s11n_exception( "deserialize_streamable_pair(): 'second' property not set!", __FILE__, __LINE__ );
	}
...
bool s11n::map::serialize_map( NodeType & dest, const MapType & src )
{
	...
	if( ! TR::empty( dest ) )
	{
		throw ::s11n::s11n_exception( "serialize_map(tgt_node=["+TR::name(dest)+"]) requires that the target node be empty.", __FILE__, __LINE__ );
	}
	TR::class_name( dest, STR::class_name(&src) );
	CIT b = src.begin();
	CIT e = src.end();
