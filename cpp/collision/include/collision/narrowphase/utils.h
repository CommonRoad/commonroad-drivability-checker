...
RectangleOBBConstPtr create_obb_from_points(Eigen::Vector2d pt1, Eigen::Vector2d pt2, double rect_width) {
  ...
  return std::make_shared<const RectangleOBB>(length / 2, rect_width / 2, local_axes, center);
...
int generate_rectangles_from_vertex_list(std::vector<Eigen::Vector2d> &verts,
  ...
  for (auto vert = verts.begin(); vert < verts.end() - 1; vert++) {
    collision::RectangleOBBConstPtr rect_new = collision::create_obb_from_points(*vert, *(vert + 1), rect_width);
    if (rect_new)
      sg_rects_ptr->addToGroup(rect_new);
  }
  if (verts.size() > 1) {
    collision::RectangleOBBConstPtr
        rect_new = collision::create_obb_from_points(*(verts.end() - 1), *(verts.begin()), rect_width);
    if (rect_new)
      sg_rects_ptr->addToGroup(rect_new);
  }
  return 0;
