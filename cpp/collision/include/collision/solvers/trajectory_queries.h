...
public:
	...
	typedef std::vector<OBB, Eigen::aligned_allocator<OBB>>::const_iterator OBBConstIterator;
	...
	OBBDenseTrajectoryBatch(std::vector<OBB, Eigen::aligned_allocator<OBB>> rectangles, int traj_length, const VectorXi& start_step):
		rectangles_(rectangles), traj_length_(traj_length), start_step_(start_step)
	{
		int num_traj=rectangles.size()/traj_length;
		if(num_traj*traj_length !=rectangles.size())
			throw;
...
private:
	...
	mutable std::vector<AABB, Eigen::aligned_allocator<AABB>>::iterator aabb_begin_;
