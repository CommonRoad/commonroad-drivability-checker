...
Segment::Segment(Eigen::Vector2d pt_1, Eigen::Vector2d pt_2, Eigen::Vector2d t_1, Eigen::Vector2d t_2) {
  this->pt_1_ = pt_1;
  this->pt_2_ = pt_2;
...
Eigen::Vector2d Segment::pt_1() const {
  return this->pt_1_;
}

Eigen::Vector2d Segment::pt_2() const {
  return this->pt_2_;
}
...
Eigen::Vector2d Segment::convertToCartesianCoords(double s, double l) const {
  ...
  Eigen::Vector2d p_local = Eigen::Vector2d(s, 0) + l * Eigen::Vector2d(-pseudo_tangent[1], pseudo_tangent[0]);
...
std::vector<EigenPolyline> Segment::convertRectangleToCartesianCoords(double s_lo_local,
                                                                      double s_hi_local,
                                                                      double l_lo,
                                                                      double l_hi) const {
...
Eigen::Vector2d Segment::normalSegmentStart() const {
  return this->n_1_;
}

Eigen::Vector2d Segment::normalSegmentEnd() const {
  return this->n_2_;
}
...
Eigen::RowVectorXd Segment::computePseudoDistance(const Eigen::RowVectorXd &lambdas, const Eigen::Matrix2Xd &points) {
  ...
  Eigen::Matrix2Xd diff_tmp = (this->pt_2_ - this->pt_1_).replicate(1, lambdas.cols());
  ...
  Eigen::RowVectorXd distance = (pseudonorm_x.cwiseProduct(pseudonorm_x)
      + pseudonorm_y.cwiseProduct(pseudonorm_y)).cwiseSqrt();
  return distance;
}

double Segment::computeLambda(double s) const {
  return s/this->length_;
}
...
Eigen::Vector2d Segment::computePseudoNormal(Eigen::Vector2d p_lambda, Eigen::Vector2d p) const {
  return p - p_lambda;
}
...
double Segment::computeSignedPseudoDistance(Eigen::Vector2d pseudo_normal, Eigen::Vector2d p_local) const {
